...
#   - playbook: a playbook in the tests directory (default = "run.yml")
...
docker run --detach --volume=$directory:/etc/ansible/playbooks/playbook_under_test:rw --env ANSIBLE_VAULT_PASSWORD_FILE=/etc/ansible/playbooks/playbook_under_test/passwordfile --name $container_id $opts $docker_owner/docker-$distro-ansible:latest $init

printf "\n"

# Install requirements if `requirements.yml` is present.
if [ -f "$directory/requirements.yml" ]; then
